...
async function updateAuthHeader() {
  ...
  targetNodes.forEach((node) => {
    ...
    if (user && isEmailVerified(user)) {
      ...
      node.innerHTML = `<a class="button secondary auth-mini" href="/account.html">${authText("Account", "내 계정")}</a>`;

    } else if (user) {

      node.innerHTML = `<a class="button secondary auth-mini" href="/account.html">${authText("Verify email", "이메일 인증")}</a>`;

    } else {

      node.innerHTML = `<a class="button secondary auth-mini" href="/auth.html">${authText("Sign in", "로그인")}</a>`;
...
async function handleLogin(event) {
  ...
  if (!isEmailVerified(user)) {
    ...
    window.location.href = "/account.html?verify=required";
  ...
  if (!user) {
    ...
    accountRoot.innerHTML = `
      ...
      <div class="auth-panel">
        ...
        <a class="button primary" href="/auth.html">${authText("Go to sign in", "로그인으로 이동")}</a>
